...
videos, we find that EndoViT leads DINOv2 by 6.0 accuracy points at the frame level
($w{=}1$; $p<0.001$), but that 85\% of this advantage disappears under causal
temporal smoothing, leaving a residual gap
...
Since EndoNet \citep{twinanda2017endonet} established the frame-level baseline,
the field has advanced almost entirely through temporal modelling. TeCNO
\citep{czempiel2020tecno} introduced the two-stage protocol---frozen or
fine-tuned frame features followed by a multi-stage temporal convolutional
network \citep{farha2019mstcn}---that remains the standard evaluation harness.
Subsequent work replaced or augmented the aggregator: temporal memory relation
networks \citep{jin2021tmrnet}, segment-level semantic modelling
\citep{ding2022segment}, hybrid embedding-aggregation transformers
(Trans-SVNet, \citealp{gao2021transsvnet}), long-video transformers (LoViT,
\citealp{liu2025lovit}), and online key-information transformers (SKiT,
\citealp{liu2023skit}). Reported accuracies now exceed 90\%, against the
0.66--0.72 we measure for frozen frame-level features.

\textbf{Framing.} This progression is itself evidence for our premise: the
architectures exist because frozen frame-level features are temporally
unreliable. What has not been asked is whether the aggregation they supply
\emph{changes which backbone is best}. We show that it does, and specifically
that it substitutes for domain pretraining. \citet{funke2023metrics} make a
complementary argument at the metric level, showing that conclusions in this
literature are sensitive to evaluation-metric choice; our results extend that
sensitivity from the metric to the aggregation regime. These
...
Surgical foundation models have proliferated rapidly and differ along two axes
relevant here: pretraining corpus and supervision signal. Vision-only
self-supervised models include EndoViT \citep{batic2024endovit}, which applies
masked autoencoding \citep{he2022mae} to Endo700k; EndoFM
\citep{wang2023endofm}; GSViT \citep{schmidgall2024gsvit}; and SurgeNetXL
\citep{jaspers2025surgenetxl}, trained on over 4.7M frames spanning four
procedures. Vision--language models include PeskaVLP \citep{yuan2024peskavlp}
and SurgCLIP \citep{perez2026surglavi}. ZEN \citep{zen2026} unifies both
families via multi-teacher distillation and reports the broadest evaluation to
date---20 tasks under fine-tuned, frozen-backbone, few-shot and zero-shot
settings. Outside surgery, BiomedCLIP \citep{zhang2023biomedclip} provides
medical-domain pretraining via language supervision on 15M biomedical
figure--caption pairs, which we include as the medical/language-supervised cell
of our $2\times2$.

Reported domain-pretraining gains are protocol-dependent in a manner directly
consistent with our finding. \citet{jaspers2025surgenetxl} report
procedure-specific pretraining improving Dice by 10.0--29.6\% over ImageNet
initialisation with \emph{frozen} weights but only 2.8--12.8\% with trainable
weights---the frozen setting inflates the apparent benefit roughly two- to
threefold on the fine-tuning axis, as we find it does on the temporal axis.

Most relevantly, \citet{zen2026} evaluate EndoViT on AutoLaparo
\citep{wang2022autolaparo}---a procedure absent from Endo700k---under the
TeCNO protocol with an MS-TCN head, and report video-level F1 of 0.636 against
0.668 for a plain ImageNet-pretrained ViT-B. Outside its pretraining domain and
under a stronger aggregator than ours, EndoViT's advantage does not merely
shrink but reverses. We return to this in \S\ref{sec:limits}. Reported gains are
...
\bibitem[Ding and Li(2022)]{ding2022segment}
X.~Ding and X.~Li.
\newblock Exploring segment-level semantics for online phase recognition from
  surgical videos.
\newblock \emph{IEEE Trans.\ Medical Imaging}, 41(11):3309--3319, 2022.
...
\bibitem[Jin et~al.(2021)]{jin2021tmrnet}
Y.~Jin, Y.~Long, C.~Chen, Z.~Zhao, Q.~Dou, and P.-A. Heng.
\newblock Temporal memory relation network for workflow recognition from
  surgical video.
\newblock \emph{IEEE Trans.\ Medical Imaging}, 40(7):1911--1923, 2021.
...
\bibitem[Schmidgall et~al.(2024)]{schmidgall2024gsvit}
S.~Schmidgall, J.~W. Kim, J.~Jopling, and A.~Krieger.
\newblock General surgery vision transformer: A video pre-trained foundation
  model for general surgery.
\newblock \emph{arXiv:2403.05949}, 2024.
...
\bibitem[Wang et~al.(2023)]{wang2023endofm}
Z.~Wang, C.~Liu, S.~Zhang, and Q.~Dou.
\newblock Foundation model for endoscopy video analysis via large-scale
  self-supervised pre-train.
\newblock In \emph{MICCAI}, pages 101--111, 2023.
...
\bibitem[Yuan et~al.(2024)]{yuan2024peskavlp}
K.~Yuan, N.~Navab, and N.~Padoy.
\newblock Procedure-aware surgical video-language pretraining with hierarchical
  knowledge augmentation.
\newblock \emph{NeurIPS}, 37:122952--122983, 2024.
